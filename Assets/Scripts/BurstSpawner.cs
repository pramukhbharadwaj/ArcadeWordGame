...
// List to choose the number of bubbles to spawn in one go.
    ...
    public void CoroutineSpawn(float initialDelayBurstSpawner, float minSpawnDelayBurstSpawner, float maxSpawnDelayBurstSpawner)
    {
        StartCoroutine(Spawn(initialDelayBurstSpawner, minSpawnDelayBurstSpawner, maxSpawnDelayBurstSpawner));
    }
...
// spawns n incorrect/wrong bubbles
    ...
    {
        float incorrectBubblesPercentage = 0.33f;
    ...
    public IEnumerator Spawn(float initialDelayBurstSpawner, float minSpawnDelayBurstSpawner, float maxSpawnDelayBurstSpawner)
    {
        givenWord = FindObjectOfType<GameManager>().initialText.text;

        yield return new WaitForSeconds(initialDelayBurstSpawner); 
    ...
    // array of correct letters.
        ...
        while (enabled)
        {
            ...
            spawnDelay = Random.Range(minSpawnDelayBurstSpawner,maxSpawnDelayBurstSpawner);
